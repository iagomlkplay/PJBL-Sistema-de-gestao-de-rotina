...
| `UsuarioGestor`      | Gestor – gerencia equipe, projetos e tarefas. Métodos: `criarProjeto()`, `criarAtribuirTarefa()`, `criarAtribuirTarefaEmProjeto()`, `validarFinalizacao()`, `processarSolicitacao()`, `reatribuirTarefaAtrasada()`, `listarSolicitacoesPendentes()`.                           |
...
| `Solicitacao` | Armazena justificativa, status (`PENDENTE`/`APROVADA`/`REJEITADA`) e o desenvolvedor solicitante.                                                                                                                                                                                     |
...
9. **Solicitações de reorganização** – Desenvolvedor chama `solicitarReorganizacao()`. O gestor lista as solicitações pendentes e decide aprovar ou rejeitar (`processarSolicitacao()`). Em caso de aprovação, o gestor pode reatribuir tarefas manualmente.
