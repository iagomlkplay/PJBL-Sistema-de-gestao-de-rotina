...
public class ProjetoDAO {
    ...
    public void inserir(Projeto projeto) throws SQLException {
        String sql = "INSERT INTO projetos (nome, prazo, importancia, status, gestor_id) VALUES (?, ?, ?, ?, ?)";
        try (Connection conn = getConnection();
            ...
            stmt.setInt(5, projeto.getGestorId());
            stmt.executeUpdate();
    ...
    public void atualizarProjeto(Projeto projeto) throws SQLException {
        String sql = "UPDATE projetos SET nome = ?, prazo = ?, importancia = ?, status = ?, gestor_id = ? WHERE id = ?";
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, projeto.getNome());
            stmt.setDate(2, new java.sql.Date(projeto.getPrazo().getTime()));
    ...
    public List<Projeto> listarPorGestor(int gestorId) throws SQLException {
        List<Projeto> lista = new ArrayList<>();
